...
export default function Reels() {
  ...
  const importInstagramMutation = useMutation({
    mutationFn: async (url: string) => {
      const response = await apiRequest("/api/reels/import-instagram", {
        method: "POST",
        body: JSON.stringify({ instagramUrl: url }),
      });
      if (!response.ok) {
        const error = await response.json();
        throw new Error(error.message || "Failed to import Instagram reel");
      }
      return response.json();
    },
    onSuccess: (data) => {
      ...
      toast({ title: "Instagram reel data imported successfully" });
    },
    onError: (error: any) => {
      setIsImporting(false);
      toast({ 
  ...
  const handleImportInstagram = () => {
    if (!instagramUrl.trim()) {
      toast({ title: "Please enter Instagram URL", variant: "destructive" });
      return;
    }
    setIsImporting(true);
    importInstagramMutation.mutate(instagramUrl);
  };

  const handleUseImportedData = () => {
    if (importedData) {
      setFormData({
        farmId: importedData.farmId,
        title: importedData.title,
        description: importedData.description,
        videoUrl: importedData.videoUrl,
        thumbnailUrl: importedData.thumbnailUrl,
        farmAliasName: importedData.farmAliasName,
          ...
          <Dialog open={isImportDialogOpen} onOpenChange={setIsImportDialogOpen}>
            ...
            <DialogContent className="max-w-md">
              ...
              <div className="space-y-4">
                <div>
                  <Label htmlFor="instagramUrl">Instagram Reel URL</Label>
                  <Input
                    id="instagramUrl"
                    value={instagramUrl}
                ...
                {importedData && (
                  <div className="p-4 border rounded-lg bg-green-50">
                    <h4 className="font-medium mb-2">✅ Data imported successfully!</h4>
                    <div className="space-y-1 text-sm">
                      <p><strong>Title:</strong> {importedData.title}</p>
                      <p><strong>Farm:</strong> {importedData.farmAliasName || 'Not detected'}</p>
                      <p><strong>Tags:</strong> {importedData.tags?.join(', ') || 'None'}</p>
                    </div>
                    <Button onClick={handleUseImportedData} className="w-full mt-3">
                      Use This Data
                    </Button>
                  </div>
                )}
